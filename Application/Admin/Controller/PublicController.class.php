...
//                    如果是vip用户
                    if($info['type'] ==2 || $info['type'] ==3){
                        ...
                        if($info['is_update'] ==1){
                            $this->logout("申请升级创客审核中，请审核成功后登录");exit;
                        }
                    }
                    if($info['status'] ==2){
    ...
    public function logout($res=''){
        if(is_login()){
            D('Member')->logout();
            session('[destroy]');
            $this->success($res?$res:'退出成功！', U('Public/login'));
        } else {
            $this->redirect('login');
        }
    }
...
//            dump($_POST);exit;
            ...
            $where_chuangke['district_id'] = array("eq",I("district_id"));
            $member_chuangke = M("member")->where($where_chuangke)->find();
            if($type==2 && $member_chuangke){
                $this->error("当前所选择城市以有创客，请重新选择市。");
            }
...
//                    ->find();
                ...
                if($package_id ==1){  //1为高级创客注册 2为vip注册
                    $member_data['type'] = 2;
                    $order_data['total'] = $package_data['package_price']; //注册高级创客
                }else{
                    $member_data['type'] = 3;
                ...
                //生成订单信息
                $uid = $_SESSION['onethink_admin']['user_auth']['uid']?$_SESSION['onethink_admin']['user_auth']['uid']:1;
...
//                $order_data['goods_id'] = implode(",",$goods_ids);
            ...
            if($type ==2){
                ...
                $package_data['title'] = '牙膏(5箱),洗衣液(10箱),瓜果清洗宝(10),产品试验箱(1个),单筒水机(2箱)';
            }else{
                $package_data = M("package")->find(2);
                $package_data['title'] = '牙膏1支,洗衣液1瓶,瓜果清洗宝1瓶,单筒水机1台';
            }
...
//        header("Content-type:text/html;charset=utf-8");
        ...
        $uid = $_SESSION['onethink_admin']['user_auth']['uid'];
        $where_member['uid'] = array("eq",$uid);
        $where_member['first'] = array("eq",1);
        $order_data = M("goods_order")->where($where_member)->find();
        $total = $order_data['total']*100;
...
//        $order = time().rand(00000,99999);
        ...
        $json =  \Pingpp\Charge::create(array('order_no'  => $order,
                'amount'    => $total,//订单总金额, 人民币单位：分（如订单总金额为 1 元，此处请填 100）
...
//                    'exter_invoke_ip '=>'103.235.232.91',
    ...
    public function successPay(){
        ...
        $data['pay_status'] = 1;
        $data['reg_status'] = 2;
        $where_member['uid'] = array("eq",$id);
        $member->where($where_member)->save($data); //更新用户状态

        $where_order['first'] = array("eq",1);
        $where_order['user_id'] = array("eq",$id);
        M("goods_order")->where($where_member)->setField("pay_status",1);
...
//        dump($long_time);exit;
        if($info['type'] ==2 && $long_time <0 ){
            ...
            $where['created'] = array("between","$min,$max");
            $where['type'] = 1;
            $score = M("pre_user_score_log")->where($where)->sum("score");
            if($score < 30000*100){
                M("member")->where("uid=".$info['uid'])->setField("status",2);
                $uids = M("user_invite_user")->where("invite_user_id=".$info['uid'])->getField("user_id",true);
...
//        $phone = I("post.phone");
        ...
        $str = rand(1000, 9999);
        $content =rawurlencode("您的验证码是：" . $str . "。有效期为90秒，如非本人操作，请忽略。【苏荷环保科技】");
