...
class ShopController extends AdminController
{
    ...
    {//单商品购买
        ...
        if ($num && $id) {
            ...
            if ($goods_stock[0]['goods_num'] >= $num) {
                ...
                $shopping = M('shopping');
                $data = $shopping->create();
                $exist = $shopping->where('user_id = ' . $uid . ' && status=0 && goods_id = ' . $id)->getField('id,goods_num,goods_price');
                if ($exist) {
    ...
    public function shopcart()
    {
        if ($_POST) {
            ...
            if ($data) {
                if($type ==2) {
                    $score_level_rule_find = $score_level_rule->where('id=2')->getField('id,source_value,return_name,return_value,return_invite_user', true);
                    $score_level_rule_values = $score_level_rule_find['2']['return_value'];
                    ...
                    $order->total = $total;
                    $order->rebate_total = $total * $score_level_rule_values/100;
                    ...
                    $score_level_rule_values = $score_level_rule_find['1']['return_value'];
                    $order->order_code = $order_code;
                    $order->user_id = $uid;
                    $order->user_score_level_rule_id = $user_score_level;
                    $order->goods_id = $id_string;
            ...
            $shopping = M('shopping');
            $uid = parent::get_uid();
            $exist = $shopping->where('user_id = ' . $uid .' and status=0')->getField('id,goods_id,goods_num,goods_price,package_id');
            if (!$exist) {
    ...
    public function shoporder()
    {
        ...
        if ($_POST) {
            ...
            $goods_order = $goods_order_m->where('order_code=' . $order_code)->getField('id,total,rebate_total,goods_id,order_code,package_id');
...
//            if(parent::get_type() ==3) {
                ...
                $total = $goods_order[0]['rebate_total'];
//            } else {
...
//            }
            ...
            if ($user_score[0]['score'] >= $total) {
                ...
                if ($user_score_m->where('id=' . $user_score[0]['id'])->save() !== false && $member->where('uid=' . $uid)->save() !== false && $user_score_log->add($data_user_score_log)) {
                    ...
                    if($user_invite_user_value && $type==2) {
                        $goods_order_m->is_rebate_total = 1;
                    }
                    ...
                    if ($goods_order_m->where('order_code=' . $order_code)->save() !== false && $goods_order_detail->where('order_id=' . $goods_order[0]['id'])->save($data_goods_order_detail) !== false) {
                        ...
                        if (($list !== false) || ($list_ !== false)) {
                            /*返利/奖励START*/
                            $type = parent::get_type();
                            $where_score_level_rule['id'] = array('in', array('1'));
                            ...
                            if ($type == 2) {
                                ...
                                $where_score_level_rule_type['type'] = 2;
                                $score_level_rule_find = $score_level_rule->where($where_score_level_rule_type)->order('id DESC')->getField('id,source_value,return_name,return_value,return_invite_user', true);
                                $score_level_rule_find_values = parent::get_list($score_level_rule_find, 'id');
                                $member_score = $member->where('uid=' . $uid)->getField('uid,score,reward_times');
                                $member_score_key = parent::array_value($member_score);
                                $user_now_reward_times = $member_score_key[0]['reward_times'];
                                $user_now_score = $member_score_key[0]['score'];
                                $user_now_score_floor = floor($user_now_score)/10000;
                                if($user_now_score_floor > $user_now_reward_times) {
                                    if($user_now_score_floor < 10) {
                                        foreach($score_level_rule_find_values as $key => $val) {
                                            if($val['source_value'] < $user_now_score && ($val['source_value']+10000) > $user_now_score) {
//                                                $result = self::RewardCalculation($uid, $val['source_value'],$val['return_invite_user'], $user_now_score_floor);//用户id，等级满足条件，返利率
                                                $result = self::RewardCalculation($uid, $total,$order_code,$val['return_invite_user'], floor($user_now_score_floor));//用户id，等级满足条件，返利率
                                                break;
                                            }else{
                                                $result = true;
                                            }
                                        }
                                    } else if($user_now_score_floor < 200) {
                                        $result = self::RewardCalculation($uid,$total,$order_code, 10,floor($user_now_score_floor));//用户id，等级满足条件，返利率
//                                        $result = self::RewardCalculation($uid, $user_now_score_floor*10000, 10, $user_now_score_floor);//用户id，等级满足条件，返利率
                                    } else if($user_now_score_floor >= 200 && $user_now_reward_times<200) {
                                        $zero_score = $member->where('status=1')->sum('score');//所有用户的消费流水总额
                                        $zero_score = intval($zero_score/10000) * 10000;
                                        $result = self::RewardCalculation($uid, $zero_score, $order_code,$score_level_rule_find_values['17']['return_invite_user'], 200);//用户id，等级满足条件，返利率
                                    } else {
                                        //不作为
                                        $result = true;
                                    }
                                ...
                                $user_invite_user_value = $user_invite_user->where('user_id=' . $uid)->getField('invite_user_id');
                                if ($user_invite_user_value) {
//                                    $where_score_level_rule_['id'] = array('in', array('1'));
//                                    $score_level_rule_find = $score_level_rule->where($where_score_level_rule_)->getField('id,source_value,return_name,return_value,return_invite_user', true);
                                    $result = self::RewardCalculationToOnline($uid, $user_invite_user_value, $goods_order[0]['rebate_total'], $score_level_rule_find['1']['return_invite_user']);
                                }
                            }
                            $user_invite_user_value = $user_invite_user->where('user_id=' . $uid)->getField('invite_user_id');
                            if ($user_invite_user_value && $type == 3) {
                                $result = self::RewardCalculationOnline($uid, $user_invite_user_value, $goods_order[0]['rebate_total'], $score_level_rule_find['1']['return_invite_user']);
                            } else if($type == 4) {
                                $result = true;
                            }
                            if ($result == true) {
            ...
            $goods_ids = explode(',', $goods_order[0]['goods_id']);
            $num = count($goods_ids);
            $where_goods_ids['user_id'] = $uid;
            $where_goods_ids['goods_id'] = array('in', $goods_ids);
            $goods_exist = $shopping->where($where_goods_ids)->limit($num)->getField('id,goods_id,goods_num,goods_price', true);
            //print_r($goods_order);
...
//            }
    ...
    //奖励计算公式：创客奖励
    public static function RewardCalculation($user_id, $score,$order_code, $return_invite_user, $reward_times)
    {
        $order = M('goods_order');
        $member = M('member');
        $user_score = M('user_score');
        $user_score_log = M('user_score_log');
        $user_score->score=$user_score->where('user_id=' . $user_id)->getField("score");
//        $user_score = self::array_value($user_score);
//        $model = new \Think\Model();
        $score_value = $score * $return_invite_user / 100;
        $user_score->score = $user_score->score + $score_value;
        $user_score->where('user_id=' . $user_id)->save() ;

        $user_score->updated = time();

        $member->reward_times = $reward_times;
        $member->updated = time();

        $order->updated=time();
        $order->return_score= $score_value;
        $data_user_score_log['user_id'] = $user_id;
        $data_user_score_log['self_year_month_day'] = date('Y-m-d', time());
        $data_user_score_log['score'] = $score_value;
        $data_user_score_log['content'] = '创客购买返利' . $score_value . '积分';
        $data_user_score_log['type'] = 5;
        $data_user_score_log['created'] = time();
        $data_user_score_log['status'] = 1;
        /*日志记录END*/
        if ($order->where('order_code=' . $order_code)->save() !== false && $user_score->where('user_id=' . $user_id)->save() !== false && $member->where('uid=' . $user_id)->save() !== false && $user_score_log->add($data_user_score_log)) {
            return true;
        } else {
            return false;
        }
    ...
    //奖励计算公式：VIP升级上去的创客对上线的返利
    public static function RewardCalculationToOnline($user_id, $invite_user, $score, $return_invite_user)
    {
        $member = M('member');
        $user_score_obj = M('user_score');
        $user_score_log = M('user_score_log');
//        $user_score = self::array_value($user_score);
        $model = new \Think\Model();
        $score_value = $score * $return_invite_user / 100;
        $find_score = $user_score_obj->where('user_id=' . $invite_user)->getField('score');
        $user_score_obj->score = $find_score + $score_value;
        $user_score_obj->updated = time();

        $data_user_score_log['user_id'] = $invite_user;
        $data_user_score_log['self_year_month_day'] = date('Y-m-d', time());
        $data_user_score_log['score'] = $score_value;
        $data_user_score_log['content'] = '拥有上线的创客购买订单返利上线,奖励' . $score_value . '积分';
        $data_user_score_log['type'] = 5;
        $data_user_score_log['created'] = time();
        $data_user_score_log['status'] = 1;
        /*日志记录END*/
            if ($user_score_obj->where('user_id=' . $invite_user)->save() !== false && $user_score_log->add($data_user_score_log)) {
            return true;
        } else {
            return false;
        }
...
//        $data_user_score_log['status'] = 1;
//        /*日志记录END*/
//        if ($user_score->where('user_id=' . $invite_user)->save() !== false && $user_score_log->add($data_user_score_log)) {
//            return true;
//        } else {
//            return false;
//        }
//    }
    ...
    public static function RewardCalculationOnline($user_id, $invite_user, $score, $return_invite_user)
    {
        ...
        ///$user_score->score = $find_score + $score_value;
        //$user_score->updated = time();
        $find_score = $member->where('uid=' . $user_id)->getField('score');
        $member->score = $find_score + $score;
        $member->updated = time();
...
//        $data_user_score_log['status'] = 1;
        /*日志记录END*/
        if ($member->where('uid=' . $user_id)->save() !== false) {
