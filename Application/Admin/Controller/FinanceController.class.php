...
class FinanceController extends AdminController {
    ...
    public function index(){
        ...
        $rebate = $sum/$score_level_rule_data['return_value']*100 - $sum;
        $list   = $this->lists('goods_order', $map, '', '', 'id,user_id,order_code,user_score_level_rule_id,total,pay_status,created,status');
...
//        $data = user_score_level_rule_id_to_user_score_level_rule_id_name($user_score_level_rule_ids);
        foreach($list as $k => $v) {
            $list[$k]['order_price'] = $v['total']/$score_level_rule_data['return_value']*100;
            $list[$k]['brbate_price'] = $list[$k]['order_price']-$v['total'];
...
//            }
    ...
    public function offline(){
        ...
        $score_level_rule_data = $score_level_rule->where('type=3')->order('id asc')->limit(1)->find();
        $map['status']  =   array('eq',1);
        $map['pay_status']  =   array('eq',1);
        //$register = $goods_order->where($map)->order('id asc')->limit(1)->find();
        $start = I('get.start');
        ...
        $sum = $goods_order->where($map)->sum('binary total');
        $rebate = $sum/$score_level_rule_data['return_value']*$score_level_rule_data['return_invite_user'];
        ...
        $list   = $this->lists('goods_order', $map, '', '', 'id,user_id,order_code,user_score_level_rule_id,total,pay_status,created,updated,status');
        package_id_to_package_id_name($list);
        $user_ids = array_unique(parent::get_ids($list, 'user_id'));
        $user_datail = user_id_to_nickname($user_ids);
        $user_score_level_rule_ids = array_unique(parent::get_ids($list, 'user_score_level_rule_id'));
        ...
        foreach($list as $k => $v) {
            $list[$k]['return_total'] = $v['total'] * $v['user_detail']['return_invite_user']/100;
        }
        $this->assign('start', $start);
        $this->assign('end', $end);
        $this->assign('rebate', $rebate);
