...
class AdminController extends Controller
{
     ...
     */
    public static function get_ids($ars, $id_name)
    {
        ...
        foreach ($ars as $k => $v) {
            if (is_object($v)) {
                ...
                //
                $en = 'sys tls format get ids value type is error';
                $cn = '在获取KEY集合时，发现结构是错误的';
                sys_exception_manager::throw_new_exception($en, $cn, SYSTEM_EXCEPTION);
            }
        }
        return $ids;
    }

    public static function array_value($data)
    {
     ...
     * @author zhl  <ahfuzl@126.com>
     */
    public static function get_list($ars, $id_name)
    {
        if (empty($ars) || (!is_array($ars))) {
            return array();
        }
        ...
        foreach ($ars as $k => $v) {
            if (is_object($v)) {
                ...
                //
                $en = 'sys tls format get list value type is error';
                $cn = '在获取列表时，发现结构是错误的';
                sys_exception_manager::throw_new_exception($en, $cn, SYSTEM_EXCEPTION);
            }
        }
    ...
    {
        return $_SESSION['onethink_admin']['user_auth']['uid'];
    }

    public static function get_type()
    {
        return $_SESSION['onethink_admin']['user_info']['type'];
    }

    public static function get_user_score_level_rule_id($uid)
    {
        //$user_score_level = M('user_score_level')->where('user_id=' . $uid)->getField('user_score_level_rule_id');
        //if (!$user_score_level)
            $user_score_level = 0;
        return $user_score_level;
    }

    public static function get_order_code($uid)
    {
        return time() . $uid . rand(0000, 9999);
    }
